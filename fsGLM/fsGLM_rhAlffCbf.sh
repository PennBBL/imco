#----------------------------------#
# Lauren Beard | RH Alff-Cbf fsGLM #
#----------------------------------#
...
homedir=/data/jux/BBL/projects/coupling/imcoScripts/surfCoupling/groupAnalysis/rhAlffCbf
...
# Set parameters
meas=coupling_coef_alff_cbf.fwhm15.fsaverage5
fwhm=15
template=fsaverage5
hemi=rh
...
for i in $(cat $homedir/designs/n831.coupling_coef_alff_cbf.fwhm15.fsaverage5.ageAtScanmean_sex_pcaslRelMeanRMSMotion_restRelMeanRMSMotion.fwhm15.fsaverage5/subjlist.txt);do
	bblid=`echo "$i" | awk -F '/' '{print $1}'`
	datexscanid=`echo "$i" | awk -F '/' '{print $2}'`
	cp /data/jux/BBL/projects/coupling/couplingSurfaceMaps/alffCbf/rh/surf/${bblid}_${datexscanid}_rh.coupling_coef_alff_cbf.fwhm15.fsaverage5 /home/lbeard/concatTest
done

mri_concat  ~/concatTest/*rh.coupling_coef_alff_cbf.fwhm15.fsaverage5 --o $homedir/$hemi.$meas.$template.n$n.mgh
