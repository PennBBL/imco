...
# for testing
#sublist = '/data/jux/BBL/projects/coupling/imcoScripts/surfCoupling/subjectList.csv'
#job = 1
#k=10
#fwhm=5
#sd=fwhm/(2*sqrt(2*log(2))) # convert to sd
#retcoef = 'coef' #ifelse(opt$retcor, 'cor', 'coef')
#template='fsaverage5'
#measures=c('thickness', 'sulc')
#overwrite=TRUE
#Sys.setenv(FREESURFER_HOME="/share/apps/freesurfer/5.3.0/")
...
if( (job<=1) & any(!sapply(neighfiles, file.exists)) ){
	...
	system(paste(paste(R.home(component = "home"), '/bin/R', sep=''), '--slave --file=/data/jux/BBL/projects/coupling/imcoScripts/surfCoupling/R/kth_neighbors_v3.R --args', templatedir, k) ) 
}
